...
const checkRole = (roles) => {
    return (req, res, next) => {
        ...
        try {
            ...
            if (!req.user) {
                return res.status(401).json(jsend.error({ message: 'Unauthorized: User not authenticated.' }));
            }
            ...
            res.status(500).json(jsend.error({ message: 'Server error while checking role', error: error.message }));
        }
    };
};
